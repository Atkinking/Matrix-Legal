#!/usr/bin/env python3
"""
RIKTIG Reverse Image Search med förbättrad ansiktsdetektering
"""
...
class RealFaceSearch:
    def __init__(self):
        self.db_path = "data/research.db"
        self._init_database()
        
        # Ladda olika ansiktsdetekterare
    ...
    def detect_faces_advanced(self, image_path):
        """Avancerad ansiktsdetektering"""
        ...
        # Metod 1: Standard frontal ansikte
        ...
        print(f"   Frontal ansikte: {len(faces1)} ansikten")
        ...
        # Metod 2: Profil ansikte
        try:
            faces2 = self.profile_cascade.detectMultiScale(
                gray,
                scaleFactor=1.1,
                minNeighbors=3,
                minSize=(20, 20)
            )
            print(f"   Profil ansikte: {len(faces2)} ansikten")
            ...
            print("   Profil detektering inte tillgänglig")
        
        # Metod 3: Olika skalor
        for scale in [1.1, 1.2, 1.3]:
            ...
            print(f"   Skala {scale}: {len(faces3)} ansikten")
            ...
            for (x, y, w, h) in faces3:
                face_data = {
                    ...
                    'confidence': self._calculate_confidence(gray, x, y, w, h),
                    'image_path': image_path,
                    'detection_method': f'scale_{scale}'
                }
                all_faces.append(face_data)
    ...
    def reverse_image_search(self, image_path, platforms=['instagram', 'facebook']):
        ...
        # Detektera ansikten
        faces = self.detect_faces_advanced(image_path)
...
def main():
    """Huvudfunktion"""
    print("🚀 RIKTIG Reverse Image Search")
    print("🔍 Instagram & Facebook")
    print("=" * 40)
    
    search = RealFaceSearch()
    ...
    if results['all_matches']:
        ...
        for i, match in enumerate(results['all_matches'], 1):
            print(f"   {i}. {match['platform']} - {match['similarity_score']:.2f}")
